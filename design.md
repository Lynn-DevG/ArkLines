...
### 技能
技能是角色收到指令后，根据判断条件执行的动作行为，它分为两个部分
- 技能指令：发出的释放指令，释放指令类型固定，所有角色通用（即四类基础类型）
    - 每个角色固定只有四个技能指令，即普通攻击、战技、连携技、终结技
    - 每个角色的每种技能指令有一个唯一的技能名称，无论行为是否变化，该名称均唯一
- 技能行为：收到特定指令后，根据条件判断选择执行的动作行为

在排轴时，技能指令和技能行为仅表示一组数据，其本身并不执行某些行为或动作逻辑；伤害计算和UI显示会根据技能行为内的信息，来设置各个时间段内的角色状态和一些动态数据，并用于进行伤害计算

一个技能包含的数据有如下这些：
- 技能指令
    - 技能ID，用于角色>技能指令的索引
    - 技能名称，用于显示的唯一名称，显示在skill palette上的名称
    - 技能指令类型，决定该技能在skill palette上所属的行为；后续扩展时会用于关联玩家按键操作，目前暂不考虑
        - 普通攻击
        - 战技
        - 连携技
        - 终结技
- 技能等级：
    ...
    - 技能等级影响技能的一系列动态数据，每个等级对应一组数据映射
        - 每个技能指令类型有一系列通用的动态数据
            - 普通攻击：各个连段的伤害倍率（atk_scale），重击的技力回复量（atb），重击的失衡值（poise）
            - 战技：技力消耗、
        - 此外，每个技能还可以自定义一些技能特有的动态数据
            - 对于特有的技能效果，需要根据实际的effect_type来确定它需要的动态数据，实际配置方式在技能行为和技能效果中进行说明
- 技能行为和效果
    - 技能行为在特定指令下，根据条件在技能持续过程中的特定节点执行的事件
...
- 技能类型
    - 除了上述动态数据外，技能类型还决定了一些固有的通用逻辑以及相关参数
    - 普通攻击
        - 只有主控角色可以释放普通攻击

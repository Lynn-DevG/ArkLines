...
### 武器
武器是角色携带的一种唯一物品，它会提供角色基础攻击力加成，并附带1~3个武器技能
每个角色最多携带1件武器，且仅能携带自身对应类型的武器
    - 关于武器类型，在配置中存在一个默认的映射关系如下
    1：单手剑
    2：施术单元
    3：双手剑
    5：长柄武器
    6：手铳
武器具有武器等级和武器技能等级两类动态数据，均可在角色携带武器后进行编辑
- 武器等级：1~99，决定了武器基础攻击力
- 武器技能等级：1~9，决定了武器各个技能的实际加成数值

武器拥有以下属性
- 基础攻击力：用于伤害计算中的武器基础攻击力部分
    - 武器攻击力是一个按类型和等级索引的映射关系，每件武器有一个映射ID（levelTemplateId），在weapon_atk_lv_mapping.csv中，一个ID对应一列数据，这一列的每一行则对应一个等级数据
- 武器技能
    - 每个武器有最多三种武器技能，且类型固定，分别为
        - 属性加成：一定提供力量、敏捷、智识、意志、主属性，这五类加成中的某一类（主属性即角色自身的主属性）
            - 固定为整数，直接加值，暂不考虑百分比加值的情况
        - 次要属性加成：提供包括攻击力、生命值、属性伤害加成（物理/法术/灼热/寒冷/电磁/自然）、源石记忆强度、终结技充能效率、暴击率、治疗效率，这些类型在内的静态数值加成
        - 潜能技能：一个特殊的技能逻辑，通常有生效条件，提供一些buff效果；这部分暂时留空，后续再添加
    - weapon_attr.csv中的type列作为武器技能的索引ID，add_type则表示了该技能的加成类型，lv1~lv9这几列表示每个技能对应1~9级的数值映射关系
    - 武器技能在干员装备武器后会附加在干员上，参与到实际伤害计算流程中来产生加成效果
        - 源石技艺强度为直接加值，加算在伤害计算时的角色源石技艺强度属性中
        - 攻击力加成为百分比加成，在伤害计算的基础伤害区中生效
        - 生命值加成也为百分比加成，所有来源的生命值加成加算后再作用于角色生命值
        - 暴击率也为直接加值，加算在伤害计算时的角色暴击率属性中
        - 治疗效率暂不考虑，属于一类特殊属性
